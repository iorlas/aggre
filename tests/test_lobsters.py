...
class TestLobstersCollectorDiscussions:
    def test_stores_posts(self, engine, mock_http, log):
        ...
        with patch("aggre.collectors.lobsters.collector.time.sleep"):
            ...
            count = collect(LobstersCollector(), engine, config.lobsters, config.settings, log)
    ...
    def test_dedup_across_runs(self, engine, mock_http, log):
        ...
        with patch("aggre.collectors.lobsters.collector.time.sleep"):
            config = make_config(lobsters=LobstersConfig(sources=[LobstersSource(name="Lobsters")]))
            count1 = collect(LobstersCollector(), engine, config.lobsters, config.settings, log)
            count2 = collect(LobstersCollector(), engine, config.lobsters, config.settings, log)
    ...
    def test_tag_filtering(self, engine, mock_http, log):
        story = lobsters_story()
    ...
    def test_no_config_returns_zero(self, engine, log):
        ...
        assert collect(collector, engine, config.lobsters, config.settings, log) == 0
...
class TestLobstersSearchByUrl:
    ...
    def test_search_caches_domain_lookups(self, engine, mock_http, log):
    ...
    def test_search_no_domain_returns_zero(self, engine, log):
        config = make_config(lobsters=LobstersConfig(sources=[LobstersSource(name="Lobsters")]))
        collector = LobstersCollector()
        assert collector.search_by_url("not-a-url", engine, config.lobsters, config.settings, log) == 0

    def test_search_caches_429_response(self, engine, mock_http, log):
