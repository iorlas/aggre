...
class TestFetchOneComments:
    ...
    @patch("aggre.workflows.comments.COLLECTORS")
    def test_passes_proxy_api_url_to_collector(self, mock_collectors, engine):
        """proxy_api_url from settings is passed directly to the collector."""
        settings = make_config(proxy_api_url="http://proxy-api:8080").settings
        disc_id = _seed_discussion_for_comments(engine, external_id="hn002")
        ...
        mock_cls.return_value.fetch_discussion_comments.assert_called_once_with(
            ...
            None,
            settings,
            proxy_api_url="http://proxy-api:8080",
        )
